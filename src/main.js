...
function renderFileList() {
  ...
  if (uploadedFiles.length === 0) {
    fileListEl.innerHTML =
      '<p style="color: var(--text-tertiary); text-align: center;">No files uploaded</p>';
...
function showResults(originalSize, compressedSize) {
  ...
  lastSavingsPercent = savingsPercent;
...
// ===== Compression Logic =====
async function compressPDF(pdfBytes, _quality) {
  try {
...
async function compressFiles() {
  ...
  try {
    ...
    const downloadHandler = () => {
      ...
      srLiveEl.textContent = t('sr.downloadStarted', { name: compressedFile.name });
      setTimeout(() => {
